...
switch ($op) {
    ...
    default:
        //navigation Alpha
        $GLOBALS['xoopsTpl']->assign('comment_view', true);
        // ************************************************************
        // Liste
        // ************************************************************
        ...
        foreach (array_keys($item_arr) as $i) {
            ...
            if (0 != $pos) {
                ...
                $GLOBALS['xoopsTpl']->assign('prev_page', "<span style='text-decoration: underline;'>&laquo;</span> <span style='font-weight: bold;'>" . _MD_TDMSPOT_ITEMPREV . '</span><br><br>' . $nav_date[$pos - 1] . " - <a href='" . $prev_link . "'>" . $nav_title[$pos - 1] . '</a>');
            }
            if ($pos != $numrows) {
                $next_link = tdmspot_generateSeoUrl($xoopsModuleConfig['tdmspot_seo_item'], $nav_ids[$pos + 1], $nav_title[$pos + 1]);
                $GLOBALS['xoopsTpl']->assign('next_page', "&nbsp;&nbsp;<span style='font-weight: bold;'>" . _MD_TDMSPOT_ITEMNEXT . "</span> <span style='text-decoration: underline;'>&raquo;</span><br><br>" . $nav_date[$pos + 1] . " - <a href='" . $next_link . "'>" . $nav_title[$pos + 1] . '</a>');
            }
            //
            //$GLOBALS['xoopsTpl']->assign('nav_page', $nav);
            ...
            foreach (array_keys($nav_parent_id) as $k) {
                //$navigation .= '<a href="viewcat.php?LT='.$nav_parent_id[$i]->getVar('id').'">' . $nav_parent_id[$i]->getVar('title') . '</a>&nbsp;>&nbsp;';
                $navigation .= "<a href='" . tdmspot_generateSeoUrl($xoopsModuleConfig['tdmspot_seo_cat'], $nav_parent_id[$k]->getVar('id'), $nav_parent_id[$k]->getVar('title')) . "'>" . $nav_parent_id[$k]->getVar('title') . '</a>&nbsp;>&nbsp;';
            }

            //trouve la categorie
            if ($cat = $catHandler->get($item_arr[$i]->getVar('cat'))) {
                $tpitem['cat_title'] = $cat->getVar('title');
                $GLOBALS['xoopsTpl']->assign('cat_title', $cat->getVar('title'));
                ...
                $GLOBALS['xoopsTpl']->assign('selectcat', tdmspot\Utility::getCategorySelect((int)$cat->getVar('id')));
...
//            $tpitem['user_name']  = $thisUser->getVar('name');
            ...
            if (!empty($matches[1])) {
                ...
                $GLOBALS['xoopsTpl']->assign('tdmspot_somaire', "['" . implode($matches[1], "','") . " ']");
            }
            ...
            if (0 != $xoopsModuleConfig['tdmspot_blsimil']) {
                ...
                foreach (array_keys($item_arr) as $i) {
                    ...
                    $GLOBALS['xoopsTpl']->append('tpitem_blsimil', ['id' => $item_arr[$i]->getVar('id'), 'cat' => $item_arr[$i]->getVar('cat'), 'indate' => $indate, 'title' => $title, 'link' => $link]);
                }
                unset($criteria);
            }
            ...
            if ($xoopsModuleConfig['tdmspot_blposter'] > 0) {
                ...
                foreach (array_keys($item_arr) as $i) {
                    ...
                    $indate = formatTimestamp($item_arr[$i]->getVar('indate'), 's');
                    $link   = tdmspot_generateSeoUrl($xoopsModuleConfig['tdmspot_seo_item'], $item_arr[$i]->getVar('id'), $item_arr[$i]->getVar('title'));
                    $GLOBALS['xoopsTpl']->append('tpitem_blposter', ['id' => $item_arr[$i]->getVar('id'), 'cat' => $item_arr[$i]->getVar('cat'), 'indate' => $indate, 'title' => $title, 'link' => $link]);
                }
            }
        }
