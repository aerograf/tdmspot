...
class fpdf
{
    ...
    //Ajout Kraven30
    public function TitreChapitre($lib)
    {
        //Titre
        $this->SetFont('', '', 12);
        $this->SetFillColor(200, 220, 255);
        $this->Cell(0, 6, "$lib", 0, 1, 'L', true);
        $this->Ln(4);
    ...
    echo $data[2]."<br>";
        if ($col['d'] == 1) {
            $this->Cell($col['w'],5,$data[formatTimeStamp($col['f'], "S")],1,0,$col['a'],$fill);
        } else {*/
            $this->Cell($col['w'], 5, Chars($data[$col['f']]), 1, 0, $col['a'], $fill);
        }
        //}
        $this->Ln();
    ...
    public function AddCol($field = -1, $width = -1, $caption = '', $align = 'L', $date = '0')
    {
    ...
    public function Table($query, $prop = [])
    {
        ...
        while ($row = $GLOBALS['xoopsDB']->fetchBoth($res)) {
            $this->Row($row);
        }
...
if($this->ProcessingTable)
    $this->TableHeader();
     ...
     global $titre;
        ...
        $this->SetLineWidth(1);
        $this->Cell($w,8,$titre,1,1,'C',true);
        $this->Ln(6);
        //Sauvegarde de l'ordonnée
        $this->y0=$this->GetY();
